...
void ImageQuilt::synthesize()
{
	...
	/********************** Looping in ZigZag Way **********************/
	for (int line = 1; line <= (2 * num_tiles - 1); line++){
	int start_col = std::max(0, line - (int)num_tiles);
	int count = std::min(std::min(line, (int)(num_tiles - start_col)), (int)num_tiles);

	for (int count_idx = 0; count_idx < count; count_idx++){
	unsigned int tile_hi = ((num_tiles < line) ? num_tiles : line) - 1 - count_idx;
	unsigned int tile_wi = start_col + count_idx;
	/*******************************************************************/
	// for (unsigned int tile_hi = 0; tile_hi < num_tiles; tile_hi++) {
	// for (unsigned int tile_wi = 0; tile_wi < num_tiles; tile_wi++) {
